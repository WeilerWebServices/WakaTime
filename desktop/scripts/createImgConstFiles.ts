...
const constantsFile = path.join(__dirname, '..', 'src', 'constants', 'imgPaths.ts');
...
const importStatements = dir
  .map(filename => `import ${getFileVarName(filename)} from "${getFileRelPath(filename)}";`)
  .join('\n');
const exportStatements = dir
  .map(filename => `export const ${getFileVarName(filename)}Path = ${getFileVarName(filename)}`)
  .join(';\n');
const comment = `/**
